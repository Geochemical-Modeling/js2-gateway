...
async def phreeqc_interceptor(
  inputFile: UploadFile,
  outputFileName: Annotated[str, Form()],
  dataFileChoice: Optional[str] = Form(None),
  customDataFile: Union[UploadFile, None] = None,
):
  ...
  try:
    ...
    raise HTTPException(
      status_code=400,
      detail="Failed to parse input file. Please double check your input file and try again!",
    )
  ...
  if customDataFile:
    ...
    if dataFileChoice not in databaseOptionList:
      raise HTTPException(
        status_code=400, detail=f"'{dataFileChoice}' is not a supported database file!"
      )
    ...
    if not os.path.exists(datFileDir):
      raise HTTPException(
        status_code=500,
        detail=f"Data file: {dataFileChoice} wasn't found in our records.",
      )

  binary_path = os.path.join(os.path.dirname(__file__), "phreeqc")

  try:
    input_data = f"{inputFileDir}\n{outputFileDir}\n{datFileDir}\n"
    ...
    return {"data": {"experiment_id": exp_id, "status": "submitted"}}
...
def get_results(experiment_id: str):
  ...
  if (
    ...
    and status["status"] != "completed (lock file missing)"
  ):
    return {
      "data": {
  ...
  output_dir = os.path.join(
    os.path.dirname(__file__), "nextTDB_workdirs", experiment_id, "output"
  )
  ...
  output_files = [
    ...
    if not f.lower().endswith(".zip") and not f.startswith(".")
  ]
