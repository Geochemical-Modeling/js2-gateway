...
export default function PhreeqcOnline() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let data = JSON.data;
      data.results = data.results.replace(/\\t/g, "\t").replace("/\\n/g", "\n").trim()
  ...
  const handleClipBoardCopy = () => {
    navigator.clipboard.writeText(data.results).then(() => {
      setCopied(true)
      setTimeout(() => {
        setCopied(false)
      }, 2000);
  ...
  return (
    <main id="main-content" className="rvt-layout__wrapper rvt-layout__wrapper--single rvt-container-sm">
      <div className="rvt-layout__content">
          ...
          data.results && (
            <div>
              ...
              <pre className="tsv-container">
                <button className="copy-button" onClick={handleClipBoardCopy}>{copied ? "Copied!" : "Copy"}</button>
                {data.results}
              </pre>
            </div>
          )
        ...
        {
          error && (
            <Alert title="Error working with Phreeqc" subtitle={error} type="error"/>
          )
        }
        ...
        >
          <FileInput accept=".pqi" legend="Input file" label="Upload file" id="input-file" onChange={handleFileChange} name="inputFile" />
          
          {/* Input for the name of the output file */}
          <div>
            <label htmlFor="text-input-default" className="rvt-label">Name of the output file <span className="rvt-color-orange-500 rvt-text-bold">*</span></label>
            <input type="text" id="text-input-default" className="rvt-text-input" name="outputFileName" minLength={3} maxLength={16} required value={formData.outputFileName} onChange={handleInputChange}/>
          </div>
            ...
            <ul className="rvt-list-plain">
              <li>
                <div className="rvt-radio">
                  <input type="radio" name="selectedDataOption" id="datFile-1" value="custom" checked={formData.selectedDataOption == "custom"} onChange={handleInputChange}/>
                  <label htmlFor="datFile-1">Upload custom database file</label>
                </div>
              </li>
              <li>
                <div className="rvt-radio">
                  <input type="radio" name="selectedDataOption" id="datFile-2" value="existing" checked={formData.selectedDataOption == "existing"}  onChange={handleInputChange}/>
          ...
          */}
          {formData.selectedDataOption === "custom" ? (
              <FileInput legend="Custom data file" label="Upload file" required id="custom-data-file" onChange={handleFileChange} name="customDataFile" accept=".dat"/>           
            ) : formData.selectedDataOption === "existing" ? (
              <div>
                <label htmlFor="select-input-default" className="rvt-label">Select a database:</label>
                <select id="select-input-default" className="rvt-select" name="selectedDataFile" defaultValue={formData.selectedDataFile}  onChange={handleInputChange}>
                  {
          ...
          <button className="rvt-button" type="submit">SUBMIT</button>
        </form>
      </div>
    </main>
  );
